...
def movies_by_actor_count(matches: List[str]) -> int:
    actor = matches[0]
    count = 0
    for movie in movie_db:
        if actor in get_actors(movie):
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (str.split("how many movies did % act in"), movies_by_actor_count),
    (["bye"], bye_action),
]
...
if __name__ == "__main__":
    ...
    assert sorted(title_after_year(["1990"])) == sorted(
        ["boyz n the hood", "dead again", "the crying game", "flirting", "malcolm x", "weapons",]
    ...
    assert movies_by_actor_count("orson welles") == 2
